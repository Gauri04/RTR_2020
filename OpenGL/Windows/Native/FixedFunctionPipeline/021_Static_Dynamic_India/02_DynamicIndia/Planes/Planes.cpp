...
void Display(void)
{
	...
	// | of plane (1)
		...
		glLoadIdentity();
		glTranslatef(-2.0f + grMoveOrangePlaneInHorizontal + (radius * cos(x)) + 0.0f, 2.0f + (radius * sin(x)) + 0.0f, -6.0f);
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef(-2.0f + grMoveOrangePlaneInHorizontal + (radius * cos(x)), 2.0f + (radius * sin(x)) + 0.0f, -6.0f); 
		glRotatef(grangleOrangePlane, 0.0f, 0.0f, 1.0f);
		glColor3f(0.5f, 0.5f, 0.5f); // gray color
		glBegin(GL_QUADS);
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef(-2.0f+ grMoveOrangePlaneInHorizontal + (radius * cos(x)), 2.0f + (radius * sin(x)) , -6.0f); // -0.10f
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef(-2.0f + grMoveOrangePlaneInHorizontal + (radius * cos(x)), 2.0f + (radius * sin(x)), -6.0f);
		glRotatef(40.0f + grangleOrangePlane, 0.0f, 0.0f, 1.0f);
		//glRotatef(40.0f, 0.0f, 0.0f, 1.0f);
		glColor3f(0.5f, 0.5f, 0.5f); // gray color
		glBegin(GL_QUADS);
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef(-2.0f + grMoveOrangePlaneInHorizontal + (radius * cos(x)), 2.0f + (radius * sin(x)), -6.0f);	// -2.0f, -0.088f
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef(-2.0f + grMoveOrangePlaneInHorizontal + (radius * cos(x)), 2.0f + (radius * sin(x)), -6.0f); // -0.15, -0.088
		glRotatef(135.0f + grangleOrangePlane, 0.0f, 0.0f, 1.0f);
		//glRotatef(135.0f, 0.0f, 0.0f, 1.0f);
		glColor3f(0.5f, 0.5f, 0.5f); // gray color
		glBegin(GL_QUADS);
	...
	/*
		...
		glLineWidth(3.0f);
		for (grrow = 1.0f; grrow <= 1.5f; grrow = grrow + 0.03f)
		{
			//for (y = 1.0f; y <= 2.0f; y = y + 0.025f)
			//{
			glBegin(GL_POINTS);
			glVertex3f(-2.0f + grMoveOrangePlaneInHorizontal + (radius * cos(x)) , 2.0f + (radius * sin(x)) + 0.0f, -6.0f);
			glVertex2f(grrow + 0.01, 1.0f);
			glEnd();
			//}
		...
		*/
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef(grMoveGreenPlaneInHorizontal + (radius * cos(-y)), -2.0f + (radius * sin(-y)), -6.0f);
...
void Update(void)
{
	...
	{
		grMoveOrangePlaneInHorizontal = grMoveOrangePlaneInHorizontal + 0.005f;
		if (grMoveOrangePlaneInHorizontal >= 4.5f)
		{
			...
			if (grbRotateOrangePlane == true)
			{
				if (grangleOrangePlane <= 450.0f)
				{
					...
					grangleOrangePlane = grangleOrangePlane + 0.5f;
				}
	...
	{
		grMoveGreenPlaneInHorizontal = grMoveGreenPlaneInHorizontal + 0.005f;
		if (grMoveGreenPlaneInHorizontal >= 2.0f)
		{
			...
			{
				grangleGreenPlane = grangleGreenPlane - 0.5f;
			}
	...
	if (grbRotateOrangePlane == false)
	{
		...
		{
			grangleOrangePlane = grangleOrangePlane + 0.150f;
		}
		else
		{
	...
	if(grbRotateGreenPlane == false)
	{
		if (grangleGreenPlane >= 0.0f)
			grangleGreenPlane = grangleGreenPlane - 0.140f;
		else
