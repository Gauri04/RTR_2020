...
void Display(void)
{
	...
	// distance between ax,ay and bx, by : bx-ax, by-ay
	grlength1 = sqrtl(pow((-1.0f - 0.0f), 2) + pow((-1.0f - 1.0f), 2));
	// distance between bx, by and cx, cy
	grlength2 = sqrtl(pow((1.0f + 1.0f), 2) + pow((-1.0f + 1.0f), 2));

	// distance between ax, ay and cx, cy
	grlength3 = sqrtl(pow((0.0f - 1.0f), 2) + pow((1.0f + 1.0f), 2));
