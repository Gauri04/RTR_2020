	...
	while(bDone == false)
	{
		while(XPending(gpDisplay))
		{
			...
			switch(event.type)
			{
				...
				case KeyPress:
					...
					switch(keysym)
					{
						...
						case XK_f :
							...
							{
								ToggleFullscreen();
								printf("\n debug in XK_F fullscreen false");
								bFullscreen = true;
							}
							else
							{
								ToggleFullscreen();
								printf("\n debug in XK_F fullscreen true");
								bFullscreen = false;
				...
				case MotionNotify:
					printf("\n debug in MotionNotify");
					break;
				
				case ConfigureNotify:								// similar to WM_SIZE
					winWidth = event.xconfigure.width;
					winHeight = event.xconfigure.height;
					printf("\n debug in ConfigureNotify");
					Resize(winWidth, winHeight);
					break;
				
				case Expose:										// similar to WM_PAINT
					//printf("\n debug in case Expose");
					break;
				
				case DestroyNotify:
					printf("\n debug in DestroyNotify");
					break;
		...
		//printf("\n debug in after XPending");
		Draw();
	}
	printf("\n debug in after while loop");
	Uninitialize();
	return(0);
}
