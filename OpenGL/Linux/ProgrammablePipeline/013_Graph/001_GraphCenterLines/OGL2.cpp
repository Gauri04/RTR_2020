...
/********* Shader Code *********/
	...
	glBufferData(GL_ARRAY_BUFFER, 2 * 2 * sizeof(int), NULL, GL_DYNAMIC_DRAW);
...
void Draw(void)
{
	...
	grLineVertices[0] = 0.0f;
	grLineVertices[1] = -3.0f;
	grLineVertices[2] = 0.0f;
	grLineVertices[3] = 3.0f;


	// bind vao
	glBindVertexArray(grgVao);
	glBindBuffer(GL_ARRAY_BUFFER, grgVbo_position);
	glBufferData(GL_ARRAY_BUFFER, sizeof(grLineVertices), grLineVertices, GL_DYNAMIC_DRAW);
	...
	grLineVertices[0] = -3.0f;
	grLineVertices[1] = 0.0f;
	grLineVertices[2] = 3.0f;
	grLineVertices[3] = 0.0f;
