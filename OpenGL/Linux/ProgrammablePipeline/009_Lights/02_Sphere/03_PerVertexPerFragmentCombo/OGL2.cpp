...
GLuint grgLdUniformPV;
GLuint grgLaUniformPV;
GLuint grgLsUniformPV;
...
void Initialize()
{
	...
	const GLchar* grvertexShaderSourceCodePV =		
		...
		"{" \
			...
			"vec3 specular = u_ls * u_ks * pow(max(dot(reflection_vector, view_vector), 0.0f), u_material_shinyness);" \
			"phong_ads_color = ambient + diffuse + specular;" \
		"}" \
		"else" \
		"{" \
...
void Draw(void)
{
	...
	if(grgbIsFragmentLighting != true)
	{
