...
// texture variables
GLuint grgTextureSmiley;
GLuint grgKeyPress = 1;
...
void Initialize()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(4 * 2 * sizeof(float)), NULL, GL_DYNAMIC_DRAW);
	...
	grgTextureSmiley = LoadBitmapAsTexture("Smiley.bmp");
...
void Draw(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, grgTextureSmiley);
	...
	glBindBuffer(GL_ARRAY_BUFFER, grgVboTextureCube);
	glBufferData(GL_ARRAY_BUFFER, 4 * 2 * sizeof(GLfloat), grtexCoord, GL_DYNAMIC_DRAW);
	glVertexAttribPointer(GR_ATTRIBUTE_TEXCOORD, 2, GL_FLOAT, GL_FALSE, 0, NULL);				// 2nd param is 2 because 2D texture has two parameters (S and T)
	glEnableVertexAttribArray(GR_ATTRIBUTE_TEXCOORD);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void Uninitialize()
{
	...
	if(grgTextureSmiley)
	{
		glDeleteTextures(1, &grgTextureSmiley);
		grgTextureSmiley = 0;
	}
