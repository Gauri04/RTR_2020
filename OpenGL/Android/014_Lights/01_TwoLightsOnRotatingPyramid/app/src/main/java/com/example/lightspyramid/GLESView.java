	...
	OnGestureListener, OnDoubleTapListener
{
	...
	private int[] grvboColorPyramid = new int[1];
	float grmaterialShinyness = 128;
	Light grlight;
	...
	private void initialize(GL10 gl)
	{
		...
		grlaUniform[0] = GLES32.glGetUniformLocation(grshaderProgramObject, "u_ld[0]");
		grlsUniform[0] = GLES32.glGetUniformLocation(grshaderProgramObject, "u_ld[0]");
		grldUniform[1] = GLES32.glGetUniformLocation(grshaderProgramObject, "u_ld[0]");
		grldUniform[1] = GLES32.glGetUniformLocation(grshaderProgramObject, "u_ld[0]");
		grldUniform[1] = GLES32.glGetUniformLocation(grshaderProgramObject, "u_ld[0]");
		grldUniform[0] = GLES32.glGetUniformLocation(grshaderProgramObject, "u_ld[0]");
		...
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, grtriangleVertices.length * 4, grverticesBuffer,
		...
		// color
		GLES32.glGenBuffers(1, grvboColorPyramid, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, grvboColorPyramid[0]);
		grbyteBuffer = null;
		grbyteBuffer = ByteBuffer.allocateDirect(grtriangleColors.length * 4);
		grbyteBuffer.order(ByteOrder.nativeOrder());
		FloatBuffer grcolorsBuffer = grbyteBuffer.asFloatBuffer();
		grcolorsBuffer.put(grtriangleColors);
		grcolorsBuffer.position(0);
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, grtriangleColors.length * 4, grcolorsBuffer,
				GLES32.GL_STATIC_DRAW);
	...
	void uninitialize()
	{
		...
		if(grvboColorPyramid[0] != 0)
		{
			GLES32.glDeleteBuffers(1, grvboColorPyramid, 0);
			grvboColorPyramid[0] = 0;
		}
